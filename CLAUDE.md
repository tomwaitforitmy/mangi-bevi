...
### About / Open Source Licenses / Costs

Profile → About (`screens/AboutScreen.js`) links to two sub-pages, both backed by static/generated
data rather than any backend call (see `specs/004-about-page/`):

- **Open Source Licenses** (`screens/OpenSourceLicensesScreen.js`) reads
  `data/generated/openSourceLicenses.json` — full license text for every production dependency
  whose license requires attribution (`scripts/lib/requiresAttribution.js` filters out only true
  public-domain-style licenses like Unlicense/CC0/WTFPL/0BSD; unrecognized licenses default to
  *included*). Regenerated by `scripts/generate-license-data.js` (via
  `license-checker-rseidelsohn`) as part of `npm run prerelease`, so it never manually drifts from
  what's actually shipped.
- **Costs & Transparency** (`screens/CostsScreen.js`) combines two sources: `data/CostsConfig.js`
  (hand-maintained real monetary costs — edit this directly when a real cost changes, e.g. the
  next Apple Developer Program payment) and `data/generated/devHoursEstimate.json` (an estimated
  developer-time cost derived from git commit history via a session-heuristic in
  `scripts/lib/estimateDevHours.js`, also regenerated every `npm run prerelease`).

Both screens accept their data via a prop that defaults to the real `require()`d file
(`OpenSourceLicensesScreen({ licenseData = ... })`, `CostsScreen({ costsConfig = ..., devHoursEstimate = ... })`)
— this is deliberate: `jest.mock()` on a transitively-required local module does not reliably
intercept the mock in this project's Jest/Babel setup (verified independent of file type), so
dependency injection via a prop default is the pattern for any new screen that reads a
generated/static data file, not module mocking.
